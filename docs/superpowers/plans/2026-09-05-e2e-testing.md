...
| test | current failure |
|---|---|
| `blocks a blacklisted operation written with extra whitespace` | exits 0 — `includes('DROP DATABASE')` misses the doubled space, so the blacklist is bypassed. Nothing is dropped: `requiresConfirmation` still sees the leading `DROP` and stops it at the confirmation gate. |
| `blocks a blacklisted operation separated by a comment` | **really drops `pg_e2e_alt`** — the blacklist misses it, and `--skip-confirmation` waives the gate that saved the case above |
| `blocks a blacklisted operation split across lines` | same hole; by now the database is already gone, so it fails on the stderr assertion rather than the exit code |
...
**The two `--skip-confirmation` cases really drop `pg_e2e_alt` while red** — that is exactly the hole they exist to prove. Tear the container down and back up before re-running anything else:
...
Run: `npx mocha --forbid-only "test/psql/query-validator.test.ts"`
Expected: FAIL — 7 failing: four in `checkBlacklist` (whitespace, newline, comment, and the string literal that must NOT be blocked) and three in `requiresConfirmation` (comment-prefixed, second-statement, and the string literal that must NOT trip the gate). The `deleted_items` case already passes — `' DELETE '` needs a space on both sides and `DELETED_ITEMS` has none.
